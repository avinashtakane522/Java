...
public class Test {
    public static void main(String[] args) {
        ...
        System.out.print("\n Start with: ");
        list.stream().map(e -> String.valueOf(e)).filter(e -> e.startsWith("2")).map(e -> Integer.valueOf(e)).distinct()
        ...
        long l = list.stream().count(); //The issue in this line of code is that .count() returns a long value, and not a Stream.
        ...
        freq.entrySet().stream().filter(e -> e.getValue() == 1).findFirst().ifPresent(e -> System.out.print(e.getKey() + " "));
        ...
        li.stream().map(i -> Math.pow(i, 3)).filter(i -> i > 100).forEach(i -> System.out.print(i+" "));
        ...
        Optional.ofNullable(list2).filter(lstr -> !lstr.isEmpty()).ifPresent(lstr -> System.out.print(lstr+" "));
